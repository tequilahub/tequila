...
def test_endianness():
    tests = ["000111", "111000", "101010", "010101", "10010010001", "111100101000010"]
...
@pytest.mark.skipif(
    condition="cirq" not in INSTALLED_SAMPLERS or "qiskit" not in INSTALLED_SAMPLERS,
    reason="need cirq and qiskit for cross validation",
)
...
@pytest.mark.parametrize(
    "case", [("X(0)Y(1)Z(4)", 0.0), ("Z(0)", 1.0), ("Z(0)Z(1)Z(3)", 1.0), ("Z(0)Z(1)Z(2)Z(3)Z(5)", -1.0)]
)
def test_paulistring_sampling(backend, case):
    ...
    assert isclose(result, case[1], 1.0e-4)


@pytest.mark.parametrize("backend", INSTALLED_SAMPLERS)
@pytest.mark.parametrize(
    "case", [("X(0)Y(1)Z(4)", 0.0), ("Z(0)X(1)X(5)", -1.0), ("Z(0)X(1)X(3)", 1.0), ("Z(0)X(1)Z(2)X(3)X(5)", -1.0)]
)
