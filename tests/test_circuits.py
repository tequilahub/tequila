...
def test_basic_gates():
    ...
    def BS(state):
        return QubitWaveFunction.from_basis_state(n_qubits=1, basis_state=state)
...
def test_unitary_gate_u2(ctrl, phi, lambd):
    ...
    c_equiv = (
        Rz(target=0, control=ctrl, angle=lambd)
...
@pytest.mark.parametrize(
    ...
    [
        (
            Rx(target=0, control=None, angle=numpy.pi / 5),
            numpy.pi / 5,
            -numpy.pi / 2,
            numpy.pi / 2,
...
def test_variable_map():
    U = (
        ...
        + Rz(angle="c", target=2)
        + H(angle="d", target=3)
        + ExpPauli(paulistring="X(0)Y(1)Z(2)", angle="e")
    )
