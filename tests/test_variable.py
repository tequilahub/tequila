...
def test_exotic_gradients(gradvar):
    ...
    t = (
        c * a**b
        + b / c
        - Objective(args=[c], transformation=np.cos)
        + f / (d * e)
        + a * Objective(args=[d], transformation=np.exp) / (f + b)
        + Objective(args=[e], transformation=np.tanh)
        + Objective(args=[f], transformation=np.sinc)
    )
    g = grad(t, gradvar)
    if gradvar == "a":
        assert np.isclose(
            g(variables),
            c(variables) * b(variables) * (a(variables) ** (b(variables) - 1.0))
    ...
    if gradvar == "b":
        assert np.isclose(
            ...
            + 1.0 / c(variables)
            - a(variables) * np.exp(d(variables)) / (f(variables) + b(variables)) ** 2.0,
        )
    if gradvar == "c":
        assert np.isclose(
            g(variables), a(variables) ** b(variables) - b(variables) / c(variables) ** 2.0 + np.sin(c(variables))
        )
    if gradvar == "d":
        assert np.isclose(
            g(variables),
            -f(variables) / (np.square(d(variables)) * e(variables))
            + a(variables) * np.exp(d(variables)) / (f(variables) + b(variables)),
        )
    if gradvar == "e":
        assert np.isclose(
            g(variables), 2.0 / (1.0 + np.cosh(2 * e(variables))) - f(variables) / (d(variables) * e(variables) ** 2.0)
        )
    if gradvar == "f":
        assert np.isclose(
            g(variables),
            1.0 / (d(variables) * e(variables))
            - a(variables) * np.exp(d(variables)) / (f(variables) + b(variables)) ** 2.0
            + np.cos(np.pi * f(variables)) / f(variables)
            - np.sin(np.pi * f(variables)) / (np.pi * f(variables) ** 2.0),
        )
