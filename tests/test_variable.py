...
from tequila.circuit.gradient import __grad_transform
...
def test_gradient():
	...
	assert __grad_transform(d, a) == 1.0
...
def test_transform_update():
	a=Variable('a',7)
	b=Variable('a.',23)
	t=Transform(transformation=operator.add,args=[a,b])
	d={'a':8,'a.':1,'a':9,'c':17}
	t.update(d)
...
def test_exotic_gradients(gradvar):
	...
	e=Variable('e',6)
	f=Variable('f',7)
	t= c*a**b +b/c -Transform(args=[c],transformation=np.cos) +f/(d*e) +a*Transform(args=[d],transformation=np.exp)/(f+b) \
	   +Transform(args=[e],transformation=np.tanh) + Transform(args=[f],transformation=np.sinc)
	g=__grad_transform(t, gradvar)
	if gradvar is 'a':
