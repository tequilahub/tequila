...
@pytest.mark.skipif(not HAS_PYZX, reason="Pyzx package not installed, test_convert_to_from_pyzx_simple not executed")
...
@pytest.mark.skipif(not HAS_PYZX, reason="Pyzx package not installed, test_convert_to_from_pyzx not executed")
@pytest.mark.parametrize(
    "variabs",
    [
...
@pytest.mark.skipif(
    not HAS_PYZX, reason="Pyzx package not installed, test_convert_to_from_pyzx_trotterized_gate not executed"
)
@pytest.mark.parametrize(
...
@pytest.mark.skipif(not HAS_PYZX, reason="Pyzx package not installed, test_convert_from_pyzx_exception not executed")
def test_convert_from_pyzx_exception():
    pyzx_circuit = pyzx.circuit.Circuit(qubit_amount=1)
    tequila_circuit = QCircuit()

    with pytest.raises(
        expected_exception=TequilaException, match="Circuit provided must be of type pyzx.circuit.Circuit"
    ):
...
@pytest.mark.skipif(
    not HAS_PYZX, reason="Pyzx package not installed, test_convert_to_from_pyzx_optimizing_circuit not executed"
)
@pytest.mark.parametrize(
