...
def test_convenience():
    ...
    assert paulis.Sp(i) == 0.5 * (paulis.X(i) + 1.0j * paulis.Y(i))
    assert paulis.Sm(i) == 0.5 * (paulis.X(i) - 1.0j * paulis.Y(i))
...
def test_from_string():
    ...
    test2 = 2.0 * paulis.X(0) - 1.0e-1 * paulis.Y(1) + 3.0 * paulis.X(1) * paulis.Z(0)
    test = numpy.linalg.norm((test1 - test2).to_matrix())
    assert numpy.isclose(test, 0.0)

    test1 = QubitHamiltonian("2.0*X(0) + -1.e-1*Y(1) + 3.0j*X(1)Z(0)")
    test2 = 2.0 * paulis.X(0) - 1.0e-1 * paulis.Y(1) + 3.0j * paulis.X(1) * paulis.Z(0)
    test = numpy.linalg.norm((test1 - test2).to_matrix())
    assert numpy.isclose(test, 0.0)
...
def test_transfer_operators():
    ...
    assert paulis.decompose_transfer_operator(
        ket=BitString.from_binary(binary="00"), bra=BitString.from_binary("00")
    ) == paulis.Qp(0) * paulis.Qp(1)
    assert paulis.decompose_transfer_operator(
        ket=BitString.from_binary(binary="01"), bra=BitString.from_binary("01")
    ) == paulis.Qp(0) * paulis.Qm(1)
    assert paulis.decompose_transfer_operator(
        ket=BitString.from_binary(binary="01"), bra=BitString.from_binary("10")
    ) == paulis.Sp(0) * paulis.Sm(1)
    assert paulis.decompose_transfer_operator(
        ket=BitString.from_binary(binary="00"), bra=BitString.from_binary("11")
    ) == paulis.Sp(0) * paulis.Sp(1)
