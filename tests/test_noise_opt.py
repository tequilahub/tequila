...
@pytest.mark.parametrize("p", numpy.random.uniform(0.1, 0.4, 1))
...
def test_bit_flip_scipy_gradient_free(p, method):
    ...
    result = tq.optimizer_scipy.minimize(
        objective=O, samples=1, backend=simulator, method=method, noise=NM, tol=1.0e-4, silent=False
    )
...
@pytest.mark.parametrize(
    "method", [tq.optimizer_scipy.OptimizerSciPy.gradient_based_methods[numpy.random.randint(0, 4, 1)[0]]]
)
...
@pytest.mark.parametrize("p", numpy.random.uniform(0.1, 0.4, 1))
@pytest.mark.parametrize(
    "method", [["TRUST-KRYLOV", "NEWTON-CG", "TRUST-NCG", "TRUST-CONSTR"][numpy.random.randint(0, 4, 1)[0]]]
)
...
def test_bit_flip_gpyopt(p, method):
    ...
    O = ExpectationValue(U=U, H=H)
    NM = BitFlip(p, 1)
    result = tq.optimizers.optimizer_gpyopt.minimize(
        objective=O, maxiter=10, samples=1, backend=simulator, method=method, noise=NM
    )
