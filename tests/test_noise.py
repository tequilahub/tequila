...
@pytest.mark.parametrize("p", numpy.random.uniform(0.0, 1.0, 1))
...
def test_bit_flip(simulator, p, controlled):
    ...
    assert numpy.isclose(E, 1.0 - p, atol=1.0e-1)


@pytest.mark.skipif(len(samplers) == 0, reason="Missing necessary backends")
@pytest.mark.parametrize("simulator", samplers)
@pytest.mark.parametrize("p", numpy.random.uniform(0.0, 1.0, 1))
@pytest.mark.parametrize("angle", numpy.random.uniform(0.0, 2 * numpy.pi, 1))
def test_rx_bit_flip_0(simulator, p, angle):
    ...
    assert numpy.isclose(E, (1 - 2 * p) * numpy.cos(angle), atol=1.0e-1)
...
def test_rx_bit_flip_1(simulator, p, angle):
    ...
    assert numpy.isclose(E, p + numpy.cos(angle) - p * numpy.cos(angle), atol=1.0e-1)
...
def test_double_cnot_bit_flip(simulator, p):
    ...
    E = simulate(O, backend=simulator, samples=1000, noise=NM)
    assert numpy.isclose(E, 2 * (p - p * p), atol=1.0e-1)
...
def test_phase_flip(simulator, p):
    ...
    NM = PhaseFlip(p, 1)
    E = simulate(O, backend=simulator, samples=1000, noise=NM)
    assert numpy.isclose(E, 1.0 - 2 * p, atol=1.0e-1)
...
def test_rz_phase_flip_0(simulator, p, angle):
    ...
    assert numpy.isclose(E, ((-1.0 + 2 * p) ** 3) * numpy.sin(angle), atol=1.0e-1)
...
def test_rz_phase_flip_1(simulator, p, angle):
    ...
    E = simulate(O, backend=simulator, variables={"a": angle}, samples=1000, noise=NM)
    print(E)
    assert numpy.isclose(E, ((1.0 - 2 * p) ** 2) * numpy.cos(angle), atol=1.0e-1)
...
def test_phase_damp(simulator, p):
    ...
    NM = PhaseDamp(p, 1)
    E = simulate(O, backend=simulator, samples=1000, noise=NM)
    assert numpy.isclose(E, numpy.sqrt(1 - p), atol=1.0e-1)
