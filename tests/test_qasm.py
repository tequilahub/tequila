...
def test_export_import_qasm_file(zx_calculus, variabs):
    variables = {"ang1": variabs[0], "ang2": variabs[1], "ang3": variabs[2], "ang4": variabs[3], "ang5": variabs[4]}
    ...
    qasm_code = export_open_qasm(tequila_circuit, variables=variables, zx_calculus=zx_calculus, filename=file_name)
    imported_circuit = import_open_qasm(qasm_code=qasm_code)
...
def test_import_qasm_with_custom_gates():
    ...
    tequila_circuit = (
        Y(target=1)
        + Z(target=5)
        + CX(target=3, control=2)
        + CX(target=3, control=0)
        + H(target=4)
        + CX(target=1, control=5)
        + CX(target=6, control=5)
        + Y(target=4)
    )
