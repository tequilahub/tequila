...
def test_one_qubit_wfn(simulator, method):
    ...
    assert numpy.isclose(result.energy, -1.0, atol=1.0e-2)
...
def test_one_qubit_shot(simulator):
    ...
    O = tq.ExpectationValue(U=U, H=H)
    result = tq.minimize(
        method="gpyopt-lbfgs", objective=O, maxiter=100, backend=simulator, samples=10000, initial_values=numpy.pi / 2
    )
