...
def test_one_qubit_shot(simulator):
    ...
    samples = 1000
    result = tq.minimize(
        objective=O, method="cobyla", backend=simulator, samples=samples, silent=True, initial_values=-0.5
    )
    assert numpy.isclose(result.energy, -1.0, atol=1.0e-1)
...
def test_gradient_free_methods(simulator, method):
    ...
    result = tq.optimizer_scipy.minimize(
        objective=-E, method=method, tol=1.0e-4, backend=simulator, initial_values=initial_values, silent=True
    )
    assert numpy.isclose(result.energy, -1.0, atol=1.0e-1)
...
def test_gradient_based_methods(simulator, method, use_gradient):
    ...
    result = tq.optimizer_scipy.minimize(
        ...
        method=method,
        tol=1.0e-3,
        method_options={"gtol": 1.0e-4, "eps": 1.0e-4, "finite_diff_rel_step": 1.0e-4},
...
def test_gradient_based_methods_qng(simulator, method):
    ...
    result = tq.optimizer_scipy.minimize(
        ...
        method=method,
        tol=1.0e-3,
        method_options={"gtol": 1.0e-3, "eps": 1.0e-4},
        initial_values=initial_values,
        silent=True,
    )
    assert numpy.isclose(result.energy, -0.612, atol=1.0e-1)


@pytest.mark.parametrize("simulator", simulators)
@pytest.mark.parametrize("method", tq.optimizer_scipy.OptimizerSciPy.hessian_based_methods)
@pytest.mark.parametrize("use_hessian", [None, "2-point", "3-point", {"method": "2-point", "stepsize": 1.0e-4}])
