from tequila.grouping.fermionic_functions import (
    obt_orb_to_so,
    tbt_orb_to_so,
    lr_decomp,
    compute_meas_alloc,
    convert_tbts_to_frags,
    obt_to_ferm,
    given_rotation,
    get_obt_tbt,
    n_elec,
...
h_ferm, obt, tbt, n_qubit, all_ops, uops, tbts, cartan_tbts = fm.get_init_ops(
    h_ferm, mol_name, "lr", spin_orb=False, save=False
)
...
psi_appr = fm.truncate_wavefunction(psis_appr[0], perc=50.0, n_qubits=n_qubit)
...
def test_fff():
    ...
    exps_new, vars_new = fm.compute_ev_var_all_ops(
        psis_fci[0], n_qubit, [obt_to_ferm(new_obt, True)] + convert_tbts_to_frags(new_tbts, True)
    )
