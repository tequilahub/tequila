...
def test_qubit_maps():
    ...
    U2 = (
        tq.gates.Ry(angle="a", target=qubit_map[0])
        + tq.gates.CNOT(qubit_map[0], qubit_map[1])
        + tq.gates.H(target=qubit_map[2])
    )
...
def test_variable_map():
    ...
    U = (
        ...
        + tq.gates.H(angle="d", target=3)
        + tq.gates.ExpPauli(paulistring="X(0)Y(1)Z(2)", angle="e")
    )
...
def test_l_addition(simulator, value=(numpy.random.randint(0, 1000) / 1000.0 * (numpy.pi / 2.0))):
    ...
    en1 = simulate(e1, variables=variables, backend=simulator) + 1.0
    an1 = np.sin(angle1(variables=variables)) + 1.0
    assert np.isclose(val, en1, atol=1.0e-4)
    assert np.isclose(val, an1, atol=1.0e-4)
...
def test_l_power(simulator, value=numpy.random.uniform(0.1, 1.9 * numpy.pi, 1)[0]):
    ...
    val = simulate(added, variables=variables, backend=simulator)
    en1 = simulate(e1, variables=variables, backend=simulator) ** 2
    an1 = np.sin(angle1(variables=variables)) ** 2.0
    assert np.isclose(val, en1, atol=1.0e-4)
    assert np.isclose(val, an1, atol=1.0e-4)
...
def test_r_power(simulator, value=numpy.random.uniform(0.1, 1.9 * numpy.pi, 1)[0]):
    ...
    val = simulate(added, variables=variables, backend=simulator)
    en1 = 2 ** simulate(e1, variables=variables, backend=simulator)
    an1 = 2.0 ** np.sin(angle1(variables=variables))
    assert np.isclose(val, en1, atol=1.0e-4)
    assert np.isclose(val, an1, atol=1.0e-4)
...
def test_ex_addition(
    ...
    assert np.isclose(val, en1 + en2, atol=1.0e-4)
    assert np.isclose(val, an1 + an2, atol=1.0e-4)


@pytest.mark.parametrize("simulator", simulators)
def test_ex_multiplication(
    simulator,
    value1=(numpy.random.randint(10, 1000) / 1000.0 * (numpy.pi / 2.0)),
...
def test_ex_power(
    ...
    U2 = gates.X(target=control) + gates.Rx(target=qubit, control=control, angle=angle2)
    e2 = ExpectationValue(U=U2, H=H2)
    added = (e1 + 1.0e-4) ** e2  # avoid zero division
    val = simulate(added, variables=variables, backend=simulator)
    en1 = simulate(e1, variables=variables, backend=simulator)
    en2 = simulate(e2, variables=variables, backend=simulator)
    an1 = np.sin(angle1(variables=variables))
    an2 = -np.sin(angle2(variables=variables))
    assert np.isclose(val, (en1 + 1.0e-4) ** en2, atol=1.0e-4)
    assert np.isclose(val, (an1 + 1.0e-4) ** an2, atol=1.0e-4)
...
def test_mixed_addition(
    ...
    an2 = -np.sin(angle2(variables=variables))
    assert np.isclose(val, en1 + en2, atol=1.0e-4)
    assert np.isclose(val, float(an1 + an2), atol=1.0e-4)
...
def test_mixed_division(
    ...
    U2 = gates.X(target=control) + gates.Rx(target=qubit, control=control, angle=angle2)
    e2 = ExpectationValue(U=U2, H=H2)
    added = e1 / (e2 + 1.0e-4)  # avoid zero division
    val = simulate(added, variables=variables, backend=simulator)
    en1 = simulate(e1, variables=variables, backend=simulator)
    en2 = simulate(e2, variables=variables, backend=simulator)
    an1 = np.sin(angle1(variables=variables))
    an2 = -np.sin(angle2(variables=variables))
    assert np.isclose(val, en1 / (en2 + 1.0e-4), atol=1.0e-4)
    assert np.isclose(val, an1 / (an2 + 1.0e-4), atol=1.0e-4)
...
def test_heterogeneous_operations_l(
    simulator,
    op,
    value1=(numpy.random.randint(1, 1000) / 1000.0 * (numpy.pi / 2.0)),
    ...
    an2 = np.sin(angle2(variables=variables))
    assert np.isclose(val, float(op(an1, en2)), atol=1.0e-4)
    assert np.isclose(en2, an2, atol=1.0e-4)


@pytest.mark.parametrize("simulator", simulators)
@pytest.mark.parametrize("op", [np.add, np.subtract, np.true_divide, np.multiply])
def test_heterogeneous_operations_r(
    simulator,
    op,
    value1=(numpy.random.randint(1, 999) / 1000.0 * (numpy.pi / 2.0)),
    ...
    assert np.isclose(val, float(op(en1, an2)), atol=1.0e-4)
    assert np.isclose(en1, an1, atol=1.0e-4)
...
def test_heterogeneous_gradient_r_add(simulator):
    ...
    assert np.isclose(en1, an1, atol=1.0e-4)
    assert np.isclose(doval, dtrue, atol=1.0e-4)
...
def test_inside(
    ...
    dtrue = an2 * (uen + den)
    assert np.isclose(en1, an1, atol=1.0e-4)
    assert np.isclose(deval, dtrue, atol=1.0e-4)
...
def test_akward_expression(
    ...
    assert np.isclose(deval, an2 * (uen + den), atol=1.0e-4)
    assert np.isclose(doval, dtrue, atol=1.0e-4)
...
def test_stacking():
    ...
    def f(x):
        return np.cos(x) ** 2.0 + np.sin(x) ** 2.0
    ...
    assert np.isclose(1.0, np.sum(output))


@pytest.mark.parametrize(
    "simulator",
    [tequila.simulators.simulator_api.pick_backend("random"), tequila.simulators.simulator_api.pick_backend()],
)
def test_stacking_quantum(
    simulator,
    value1=(numpy.random.randint(10, 1000) / 1000.0 * (numpy.pi / 2.0)),
    value2=(numpy.random.randint(10, 1000) / 1000.0 * (numpy.pi / 2.0)),
):
...
def test_total_type_jumble(
    ...
    totb = np.sum(np.array(lb))
    gan1 = (
        np.cos(a(values)) * appendage
        + (np.sin(a(values)) * -np.sin(b(values)))
        - (np.sin(b(values)) * -np.sin(b(values)))
    )
