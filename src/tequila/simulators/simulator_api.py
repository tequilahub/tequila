...
def compile(objective: typing.Union['Objective', 'QCircuit'],
    ...
    if isinstance(objective, Objective) or hasattr(objective, "args"):
        return compile_objective(objective=objective, variables=variables, backend=backend)
    elif hasattr(objective, "gates"):
        return compile_circuit(abstract_circuit=objective, variables=variables, backend=backend, *args, **kwargs)
    else:
        raise TequilaException(
