...
def reverse_density(density, n_qubits):
    ...
    for i in range(n_qubits // 2):
        density_qubits = numpy.swapaxes(
            numpy.swapaxes(density_qubits, i, n_qubits - i - 1), n_qubits + i, 2 * n_qubits - i - 1
        )
...
class DensityMatrix:
    ...
    def __add__(self, other):
        if self.n_qubits != other.n_qubits:
            raise TequilaException(
                "Incompatible densities of {} and {} qubits added.".format(self.n_qubits, other.n_qubits)
            )
    ...
    def __sub__(self, other):
        if self.n_qubits != other.n_qubits:
            raise TequilaException(
                "Incompatible densities of {} and {} qubits subtracted.".format(self.n_qubits, other.n_qubits)
            )
    ...
    def __rmult__(self, other):
        if self.n_qubits != other.n_qubits:
            raise TequilaException(
                "Incompatible densities of {} and {} qubits multiplied.".format(self.n_qubits, other.n_qubits)
            )
    ...
    def __repr__(self):
        return "Tequila DensityMatrix over {} qubits stored as sparse matrix\n{}".format(
            self.n_qubits, self.get_density_as_array().__repr__()
        )
