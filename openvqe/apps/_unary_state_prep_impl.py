...
class UnaryStatePrepImpl:
    ...
    def get_CNOT_moves(self, s, i1, i2):
        ...
        for target in targets:
            ...
            for index in range(len(s[i1])):   #index of a bit in the string that could possibly be a control.
                if index != target and s[i1][index] != s[i2][index] and (s[i1][index] == '0' or s[i1][index] == '1') and (s[i2][index] == '0' or s[i2][index] == '1'): #control != target & != other string & != 'a'
                    i = 0     #use i to loop through strings to check for control
                    flag = True
                    while i < len(s_to_check) and flag == True:
                        if s[i1][index] == s_to_check[i][index] or not(s_to_check[i][index] == '0' or s_to_check[i][index] == '1'):   #making sure its a 1 or 0
                    ...
                    while i < len(s_to_check) and flag == True:
                        if s[i2][index] == s_to_check[i][index] or not(s_to_check[i][index] == '0' or s_to_check[i][index] == '1'):
                            flag = False
                        i += 1
    ...
    def get_CNOT_move_non_opt(self, s, i1, i2, target_index):   #TODO: make this more opt
        ...
        for index in range(len(s[i1])):
            ...
            if index != target_index and s[i1][index] != s[i2][index] and (s[i1][index] == '0' or s[i1][index] == '1') and (s[i2][index] == '0' or s[i2][index] == '1'):
                return index
    ...
    def get_a_compression(self,s, i1, i2):
        global alph_index
        target_index = self.calc_H_distance(s[i1], s[i2])[1][0]  #collect target index, the bit where they are different
        move_param = None
        ...
        string = s[i1]
        string = list(string)
        string[target_index] = self.alphabets[self.alph_index]
        self.alph_index += 1
        string = "".join(string)
        s[i1] = string
    ...
    def get_next_move(self, s, moves_list):
        ...
        if first_key == 1:   #need a rot move
            ...
            if len(possible_rot_controls) > 0:   #you can do a rotation
                ...
                strings_moves = self.make_move(strings, index1, index2, move, 'ROT')  #fcn makes move and change all strings
            ...
            for index in range(len(strings[index1])):
                if not(strings[index1][index] == '0' or strings[index1][index] == '1') and not(strings[index2][index] == '0' or strings[index2][index] == '1'):
                    a_count1 += 0
            ...
            if (a_count1  > 0 or a_count2 > 0 ):  #TODO: test if there will be problems if bth strings contain 'a' @ same position
                if a_count1 > a_count2: first_key -= a_count1
                else: first_key -= a_count2
            ...
            if len(poss_c_moves[target_bit_index]) == 0:
                ...
                control_type = strings[index2][control_bit_index]  #can choose strings[index1] or [index2] -> make more optimal by checking which one doesn't affect a lot of other strings
    ...
    def get_circuit(self,s):
        ...
        alphabets = ['a'+str(i) for i in range(len(s)-1)]
