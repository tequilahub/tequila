    ...
    for k, v in angles.items():
        if (variables is None) or (k in variables):
            ...
            for index in indices:
                gradient[k] += __make_gradient_component(unitary=unitary, index=index)
...
def __make_gradient_component(unitary: QCircuit, index: int):
    """
    ...
    dg = []
    if isinstance(g, RotationGateImpl) or isinstance(g, ExponentialPauliGateImpl):
        if g.is_controlled():
            angles_and_weights = [
